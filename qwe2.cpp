...
const map<string, string> renames = {
    ...
    {":1208", ":binlist"}, // :binlist x y = [bit(0, x), ..., bit(y - 1, x)]
};
...
Term *list_int_term(vector<int> a) {
    Term *term = new Term("nil");
    reverse(all(a));
    for (int x: a) term = new_ap(new_ap(new Term("cons"), new Term(to_string(x))), term);
    return term;
}
    ...
    for (const auto& rule : typed_rules_tokens) {
        ...
        if (doesRuleFit(rule, keyterm, ptr, term)) {
            ...
            try {
                ...
                } else if (keyterm == ":binlist") {
                    auto xli = stoli(vars["x"]->name);
                    auto yli = stoli(vars["y"]->name);
                    assert(xli >= 0 && yli >= 0);
                    vector<int> bits(yli);
                    for (int i = 0; i < yli; ++i) bits[i] = xli % 2, xli /= 2;
                    term = list_int_term(bits);
                } else assert(false);
                term->is_non_recursive = true;
                res = true;
...
int main() {
    ...
    auto cmd = buildTerm(split("ap ap galaxy ap ap cons 1 ap ap cons ap ap cons 1 nil ap ap cons 0 ap ap cons nil nil ap ap cons 0 0"));
...
//          cerr << cmd << "\n";
        ...
        for (int i = 0; i < 100000; ++i) {
            ...
            if (i % 100 == 0) {
                cerr << i << " its\n";
                for (auto w: subs) cerr << w.first << ' ' << w.second << '\n';
            }
            if (i % 10 == 0) while (replaceAllRules(cmd)) {}
        }
        cerr << cmd << "\n";
        break;
    }
